...
class SpillGel {
  constructor(x, y, r) {
    ...
    this.r = r;
    this.stored = 0;
    this.wanderOffset = random(1000); // 用于生成平滑的无规律漂移
  }
    ...
    for (let i = particlesArray.length - 1; i >= 0; i--) {
      ...
      if (!p.inCup) {
        ...
        if (d < this.r + p.r) {
          ...
          particlesArray.splice(i, 1); // 从物理世界抹除
        }
      }
    }
  }
  ...
  squeeze(particlesArray) {
    if (this.stored > 0) {
      ...
      for (let i = 0; i < releaseCount; i++) {
        ...
        // 赋予向下的初速度，模拟被“挤”出来的感觉
        p.vel = createVector(random(-1.5, 1.5), random(2, 5));
        // 可以把挤出来的废咖啡颜色调暗一点 (RGB = 150)
        // p.col = 180;
        particlesArray.push(p);
      }
      this.stored -= releaseCount;
      this.r = this.baseR + this.stored * 0.5; // 体积缩回去
    }
  }

  display() {
    noStroke();
    // 基础颜色是半透明工业绿，吸水越多越不透明
