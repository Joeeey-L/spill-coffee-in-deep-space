...
function updateEmission() {
  ...
  frameCount_emit++;
  if (frameCount_emit % emit_rate !== 0 || particles.length >= max_particles) return;
...
function remainingDrops() {
  let remaining = 0;
  for (let e of emitters) {
    remaining += max(0, e.maxDrops - e.emittedCount);
  }
  return remaining;
}
...
class Emitter {
  ...
  emit() {
    if (this.emittedCount >= this.maxDrops) return false;
  ...
  squeeze(particlesArray) {
    if (this.stored > 0) {
      ...
      for (let i = 0; i < releaseCount; i++) {
        ...
        let p = new Particle(this.pos.x + random(-6, 6), this.pos.y + releaseOffset, r);
...
function drawInfo() {
  ...
  text("In cup: " + getGoalProgress() + " / " + goal, 10, 65);
  text("Remaining drops: " + remainingDrops(), 10, 85);
  text("Gravity: " + nf(params.gravity_x, 1, 3) + ", " + nf(params.gravity_y, 1, 3), 10, 105);
  ...
  text("Move mouse to carry the cup. A / D to tilt. Press R to restart level.", 10, height - 20);
