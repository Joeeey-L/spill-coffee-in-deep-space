...
const GAMEPAD_CURSOR_SPEED = 12;
const GAMEPAD_ROTATION_SPEED = 0.035;
const GRAVITY_SHUFFLE_PROMPT_FRAMES = 150;
const GEL_LAYOUT = [
  { x: 0.2, y: 0.32, r: 40 },
  { x: 0.8, y: 0.52, r: 40 }
];
...
const ORDER_RECIPES = [
  { key: "espresso", label: "Espresso", recipeImage: "espresso.png", ingredients: { espresso: 10 } },
  { key: "americano", label: "Americano", recipeImage: "americano.png", ingredients: { espresso: 10, water: 30 } },
  { key: "macchiato", label: "Macchiato", recipeImage: "macchiato.png", ingredients: { espresso: 10, milkFoam: 10 } },
  { key: "flatWhite", label: "Flat White", recipeImage: "flatwhite.png", ingredients: { espresso: 10, steamedMilk: 30 } },
  { key: "latte", label: "Latte", recipeImage: "latte.png", ingredients: { espresso: 10, steamedMilk: 40, milkFoam: 10 } }
];
...
let gravityShufflePromptFrames = 0;
...
function preload() {
  ...
  img_sponge = loadImage('assets/sponge.png');
  img_invitation = loadImage('assets/invitation.png');

  for (let recipe of ORDER_RECIPES) {
    recipeCards[recipe.key] = loadImage('assets/recipe/' + recipe.recipeImage);
  }
}
...
function getParticleAttractStrength(particle) {
  let material = MATERIAL_LIBRARY[particle.kind];
  return material ? material.attractStrength : params.attract_strength;
}

function isSwitchGamepad(gamepad) {
  let id = gamepad && gamepad.id ? gamepad.id.toLowerCase() : "";
  return id.includes("switch") || id.includes("pro controller") || id.includes("nintendo") || id.includes("057e");
}
...
function getActiveGamepad() {
  ...
  for (let gamepad of gamepads) {
    if (!gamepad) continue;
    if (!fallback) fallback = gamepad;
    if (gamepad.mapping === "standard" || isSwitchGamepad(gamepad)) return gamepad;
  }
  return fallback;
}

function getGamepadButtonMap(gamepad) {
  if (isSwitchGamepad(gamepad)) {
    return { confirm: 1, restart: 0, invitation: 2, plus: 9 };
  }
  return { confirm: 0, restart: 1, invitation: 3, plus: 9 };
}

function readStickAxis(value) {
  if (abs(value) < GAMEPAD_DEADZONE) return 0;
  return value;
}

function isGamepadButtonDown(index) {
  let button = gamepadInput.buttons[index];
  return !!button && button.pressed;
}

function wasGamepadButtonPressed(index) {
  let button = gamepadInput.buttons[index];
  let prevButton = gamepadInput.prevButtons[index];
  return !!button && button.pressed && (!prevButton || !prevButton.pressed);
}

function updateGamepadInput() {
  gamepadInput.prevButtons = gamepadInput.buttons.map((button) => ({ ...button }));

  let gamepad = getActiveGamepad();
  if (!gamepad) {
    gamepadInput.connected = false;
    gamepadInput.active = false;
    gamepadInput.rotateX = 0;
    gamepadInput.buttons = [];
    return;
  }

  if (gamepadInput.cursorX === 0 && gamepadInput.cursorY === 0) {
    gamepadInput.cursorX = width / 2;
    gamepadInput.cursorY = height / 2;
  }

  gamepadInput.connected = true;
  gamepadInput.name = gamepad.id || "Gamepad";
  gamepadInput.buttons = Array.from(gamepad.buttons, (button) => ({
    pressed: button.pressed,
    value: button.value
  }));

  let buttonMap = getGamepadButtonMap(gamepad);
  let leftX = readStickAxis(gamepad.axes[0] || 0);
  let leftY = readStickAxis(gamepad.axes[1] || 0);
  let rightX = readStickAxis(gamepad.axes[2] || 0);
  gamepadInput.rotateX = rightX;

  if (
    leftX !== 0 ||
    leftY !== 0 ||
    rightX !== 0 ||
    isGamepadButtonDown(buttonMap.confirm) ||
    isGamepadButtonDown(buttonMap.invitation) ||
    isGamepadButtonDown(buttonMap.restart) ||
    isGamepadButtonDown(buttonMap.plus)
  ) {
    gamepadInput.active = true;
  }

  if (gamepadInput.active) {
    gamepadInput.cursorX = constrain(gamepadInput.cursorX + leftX * GAMEPAD_CURSOR_SPEED, 0, width);
    gamepadInput.cursorY = constrain(gamepadInput.cursorY + leftY * GAMEPAD_CURSOR_SPEED, 0, height);
  }

  if (!showInvitation && wasGamepadButtonPressed(buttonMap.confirm)) {
    handlePrimaryAction(gamepadInput.cursorX, gamepadInput.cursorY);
  }

  if (wasGamepadButtonPressed(buttonMap.invitation)) {
    showInvitation = true;
  }

  if (wasGamepadButtonPressed(buttonMap.restart) || wasGamepadButtonPressed(buttonMap.plus)) {
    if (showInvitation) {
...
function handlePrimaryAction(x, y) {
  ...
  if (gameState === "playing") {
    for (let g of gels) {
      let d = dist(x, y, g.pos.x, g.pos.y);
      if (d < g.r) {
        g.squeeze(particles);
        return;
      }
...
function refreshGravityMode() {
  ...
  pulseVector = null;
  pulseIndex = 0;
  gravityShufflePromptFrames = GRAVITY_SHUFFLE_PROMPT_FRAMES;
}
...
function draw() {
  ...
  for (let i = 0; i < particles.length; i++) {
    for (let j = i + 1; j < particles.length; j++) {
      ...
      if (dSq > minDist * minDist) {
        ...
        let attractStrength = (getParticleAttractStrength(a) + getParticleAttractStrength(b)) * 0.5;
        let strength = (attractStrength * a.r * b.r) / (d * d);
  ...
  let cupTargetX = gamepadInput.active ? gamepadInput.cursorX : mouseX;
  let cupTargetY = gamepadInput.active ? gamepadInput.cursorY : mouseY;

  cup.setPos(cupTargetX, cupTargetY, particles);
  let currentMoveSpeed = 0;
  let gamepadRotation = gamepadInput.active ? gamepadInput.rotateX : 0;

  if (gamepadRotation !== 0) {
    currentMoveSpeed = gamepadRotation * GAMEPAD_ROTATION_SPEED;
    cup.idleRotation = gamepadRotation < 0 ? -0.005 : 0.005;
  }
...
function drawGravityShufflePrompt() {
  if (!activeGravityMode || gravityShufflePromptFrames <= 0) return;
...
function drawImageContained(img, x, y, maxW, maxH) {
  if (!img) return;

  let scaleFactor = min(maxW / img.width, maxH / img.height);
  let drawW = img.width * scaleFactor;
  let drawH = img.height * scaleFactor;
  image(img, x, y, drawW, drawH);
}

function drawInvitationPrompt() {
  let iconX = width - 178;
  let iconY = 22;
  ...
  stroke(255);
  strokeWeight(1.5);
  rect(iconX, iconY, 26, 18, 3);
  line(iconX, iconY, iconX + 13, iconY + 10);
  line(iconX + 26, iconY, iconX + 13, iconY + 10);

  noStroke();
  fill(255);
  textAlign(LEFT, CENTER);
  textSize(13);
  text("Invitation: Space / Y", iconX + 36, iconY + 9);
  pop();
}

function drawInvitationScreen() {
  if (!showInvitation || !img_invitation) return;
  ...
  drawImageContained(img_invitation, width / 2, height / 2 - 18, width * 0.82, height * 0.78);
  ...
  textAlign(CENTER, CENTER);
  textSize(16);
  text("Press Space or B/+ to return to the game", width / 2, height - 34);
  pop();
}
...
class Cup {
  ...
  display() {
    ...
    if (currentOrder) {
      ...
      textAlign(CENTER, CENTER);
      textSize(16);
      text("Cup: " + cupStats.total + " / " + currentOrder.totalTarget, this.pos.x, this.pos.y);
      pop();
    }
  }
}
...
class Emitter {
  ...
  display() {
    ...
    if (this.kind !== "default") {
      ...
      textAlign(CENTER, CENTER);
      textSize(12);
      text(this.label, this.pos.x, this.pos.y - 28);

      if (currentOrder && currentOrder.ingredients[this.kind] !== undefined) {
        let actual = cupStats[this.kind] || 0;
        let target = currentOrder.ingredients[this.kind];
        text(actual + " / " + target, this.pos.x, this.pos.y + 28);
      }
    }
  }
}
...
class SpillGel {
  ...
  display() {
    ...
    rotate(this.angle);
    imageMode(CENTER);
    tint(255, constrain(100 + this.stored * 10, 100, 255));
    image(img_sponge, 0, 0, this.r * 2.2, this.r * 2.4);
    noTint();
    pop();
...
function drawInfo() {
  ...
  let leftX = 12;
  let bottomY = height - 62;
  noStroke();
  fill(255);
  textAlign(LEFT, BASELINE);

  textSize(24);
  text("Orders served: " + servedOrders, 12, 38);
...
function mousePressed() {
  handlePrimaryAction(mouseX, mouseY);
}
